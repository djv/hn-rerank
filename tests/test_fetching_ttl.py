...
async def test_candidate_cache_ttl_logic():
    ...
    with (
        patch("api.fetching.datetime") as mock_dt,
        ...
        mock_get_cache.return_value = (
            None  # Always miss, so we can see what TTL was requested
        )
        ...
        print(
            f"Found TTLs: Short={found_short_ttl}, Weekly={found_weekly_ttl}, Archive={found_archive_ttl}"
        )
