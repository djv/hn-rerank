...
async def test_generate_html_integration(tmp_path):
    ...
    # Setup mocks
    with patch("generate_html.HNClient") as mock_client_class, \
        ...
        client_instance.fetch_user_data.return_value = {"pos": {1}, "neg": set()}
        ...
        # Mock sys.argv
        with patch.object(sys, 'argv', ["generate_html.py", username, "-o", str(output_file)]):
            await main()
