...
def test_rank_stories_progress_completes_after_cross_encoder(monkeypatch):
    ...
    rank_stories(
        stories,
        pos_emb,
        config=AppConfig(cross_encoder=CrossEncoderConfig(enabled=True, top_n=2)),
...
def test_cross_encoder_fallback_uses_top_cluster_titles_only(monkeypatch):
    stories = _make_stories(1)
    stories[0].text_content = "candidate body"
    positive_stories = [
        Story(
            id=i + 10,
            title=f"Title {i}",
    ...
    monkeypatch.setattr("api.rerank.get_embeddings", lambda *args, **kwargs: cand_emb)
    monkeypatch.setattr(
        "api.rerank.cluster_interests_with_labels",
        lambda *args, **kwargs: (np.array([[1.0, 0.0]], dtype=np.float32), np.zeros(len(pos_emb), dtype=np.int32)),
    )
    monkeypatch.setattr("api.rerank.init_ce_model", lambda *args, **kwargs: object())
    monkeypatch.setattr(
        "api.rerank._score_cross_encoder_candidate",
        lambda *, queries_text, **kwargs: captured_queries.append(list(queries_text)) or 0.5,
    )

    rank_stories(
        stories,
        pos_emb,
        config=AppConfig(cross_encoder=CrossEncoderConfig(enabled=True, top_n=1)),
...
def test_cross_encoder_named_query_omits_representative_title(monkeypatch):
    stories = _make_stories(1)
    positive_stories = [
        Story(
            id=10,
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            ...
            np.zeros(len(pos_emb), dtype=np.int32),
        ),
    )
    monkeypatch.setattr("api.rerank.init_ce_model", lambda *args, **kwargs: object())
    monkeypatch.setattr(
    ...
    rank_stories(
        ...
        config=AppConfig(cross_encoder=CrossEncoderConfig(enabled=True, top_n=1)),
        positive_stories=positive_stories,
        cluster_names={0: "Systems Programming"},
        cluster_keywords={0: "rust, compiler, borrow checker"},
    )

    assert captured_queries == [["Systems Programming: rust, compiler, borrow checker"]]


def test_cross_encoder_enabled_keeps_externals_outside_ce_slice(monkeypatch):
    stories = _make_stories(4)
    stories[3].source = "rss"
    positive_stories = _make_stories(1)
    pos_emb = np.array([[1.0, 0.0]], dtype=np.float32)
    cand_emb = np.array(
        [[1.0, 0.0], [0.9, 0.0], [0.8, 0.0], [0.7, 0.0]],
        dtype=np.float32,
    )
    ...
    assert [stories[r.index].source for r in results] == ["hn", "hn", "rss"]
    assert [r.cross_encoder_score > 0 for r in results] == [True, True, False]
...
def test_cross_encoder_score_cache_ignores_corrupt_file(monkeypatch, tmp_path):
    ...
    assert rerank._score_cross_encoder_candidate(
        ...
        model_dir="model-a",
        candidate_text="candidate",
        queries_text=["query a"]) == pytest.approx(0.7)
    assert calls == [1]
...
def test_classifier_metadata_features_still_allow_model_output():
    """Verify semantic_score equals model probability when metadata features are used."""
    stories = [
        Story(id=1, title="Low points", url=None, score=1, time=1000,
              text_content="A", comment_count=1),
        Story(id=2, title="High points", url=None, score=10000, time=1000,
              text_content="B", comment_count=1000),
    ]
    pos_stories = _make_stories(5)
    neg_stories = _make_stories(5)
    for s in pos_stories:
        s.comment_count = 10
    for s in neg_stories:
        s.comment_count = 1
    ...
    from api.config import CrossEncoderConfig
    config = AppConfig(
    ...
    with (
        ...
        results = rank_stories(
            stories, pos_emb, neg_emb, config=config,
            positive_stories=pos_stories, negative_stories=neg_stories,
        )

    # semantic_score is directly the model probability; hybrid_score == semantic_score
    assert results[0].semantic_score == pytest.approx(0.6, abs=1e-5)
    assert results[0].hybrid_score == pytest.approx(0.6, abs=1e-5)
    assert results[1].semantic_score == pytest.approx(0.5, abs=1e-5)
