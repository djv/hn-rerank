...
def test_classifier_metadata_features_width():
    ...
    config_none = AppConfig(classifier=ClassifierConfig(features=()))
    ...
    config_points = AppConfig(classifier=ClassifierConfig(features=("log_points",)))
    ...
    config_comments = AppConfig(
        classifier=ClassifierConfig(features=("log_points", "log_comments"))
    )
    ...
    config_all = AppConfig(
        classifier=ClassifierConfig(
            features=("log_points", "log_comments", "comment_ratio")
        )
    )
...
def test_classifier_metadata_features_values():
    ...
    assert np.all(
        feats[:, 0] <= 1.0
    )  # log points normalizer cap is high, so it's < 1.0
    assert np.all(
        feats[:, 1] <= 1.0
    )  # log comments normalizer cap is 500, so 100 is < 1.0
...
def test_classifier_metadata_comments_count_uses_comment_count_field():
    ...
    config = AppConfig(
        classifier=ClassifierConfig(
            features=("comments_count",), raw_embedding_features=False
        )
    )
