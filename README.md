...
- **Multi-Interest Clustering**: Automatically discovers your interest clusters from upvote history using Agglomerative Clustering with Ward linkage. Cluster names generated by local LLM (ollama).
...
- **Story TL;DRs**: LLM-generated 1-sentence summaries for each recommended story.
...
2. **Setup Models**:
   ```bash
   uv run setup_model.py        # BGE embeddings (ONNX)
   ollama pull llama3.2:3b      # Cluster naming (optional)
   ```
...
3. **Generate Dashboard**:
   ...
   ```
   Outputs:
   - `index.html` - Ranked story recommendations with cluster tags
